...
namespace Obsidian.Tests
{
    public class Commands
    {
        ...
        public async Task TestCommandExec()
        {
            ...
            await cmd.ProcessCommand(new CommandContext("/ping 69 hello", null, null, CommandIssuers.Client));
            Assert.Equal(69, Command.arg1out);
            Assert.Equal("hello", Command.arg2out);

            await cmd.ProcessCommand(new CommandContext("/pong ping 420 bye", null, null, CommandIssuers.Client));
            Assert.Equal(420, Command.arg1out);
            Assert.Equal("bye", Command.arg2out);

            await cmd.ProcessCommand(new CommandContext("/ping 12 12", null, null, CommandIssuers.Client));
            Assert.Equal(69, Command.arg1out);
            Assert.Equal("bye", Command.arg2out);

            await cmd.ProcessCommand(new CommandContext("/ping 69 hey bye", null, null, CommandIssuers.Client));
            Assert.Equal(69, Command.arg1out);
            Assert.Equal("bye", Command.arg2out);
        }
