...
namespace Obsidian.Commands.Framework
{
    public class CommandHandler
    {
        ...
        private const CommandIssuers DefaultIssuerScope = CommandIssuers.Any;

        internal readonly List<Command> Commands;
        internal readonly CommandParser CommandParser;
        internal readonly List<BaseArgumentParser> ArgumentParsers;
        internal readonly string Prefix;
        internal PluginManager PluginManager;

        public CommandHandler(string prefix)
        {
            this.CommandParser = new CommandParser(prefix);
            this.Commands = new List<Command>();
            this.ArgumentParsers = new List<BaseArgumentParser>();
            this.Prefix = prefix;
            ...
            {
                ArgumentParsers.Add((BaseArgumentParser)Activator.CreateInstance(parser));
            }
        }
        ...
        {
            var parserType = ArgumentParsers.FirstOrDefault(x => x.GetType().BaseType.GetGenericArguments()[0] == type)?.GetType();
        ...
        public void RegisterSingleCommand(Action method, Plugin plugin, Type t)
        {
            var m = method.Method;
        ...
        public void UnregisterPluginCommands(Plugin plugin)
        {
            this.Commands.RemoveAll(x => x.Plugin == plugin);
        }

        public void RegisterCommandClass<T>(Plugin plugin, T instance) => RegisterCommandClass(plugin, typeof(T), instance);

        public void RegisterCommandClass(Plugin plugin, Type type, object instance = null)
        {
            RegisterSubgroups(type, plugin);
            RegisterSubcommands(type, plugin, instance);
        }

        public object CreateCommandRootInstance(Type type, Plugin plugin)
        {
            ...
            // {
            //     if(injectable.PropertyType == typeof(TargetObject) || injectable.PropertyType == plugin.TargetObject.GetType())
            //     {
            //         injectable.SetValue(instance, plugin.TargetObject);
            //     }
            //     else
            //     {
            //         PluginManager.PluginServiceProvider.InjectServices(instance, plugin, PluginManager.Logger);
            //     }
            // }
        ...
        private void RegisterSubgroups(Type type, Plugin plugin, Command parent = null)
        {
        ...
        private void RegisterSubcommands(Type type, Plugin plugin, object instance, Command parent = null)
        {
        ...
        public async Task ProcessCommand(CommandContext ctx)
        {
            ...
            if (CommandParser.IsCommandQualified(ctx.Message, out ReadOnlyMemory<char> qualified))
            {
