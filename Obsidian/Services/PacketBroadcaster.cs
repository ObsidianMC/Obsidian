...
public sealed class PacketBroadcaster : BackgroundService, IPacketBroadcaster
{
    ...
    public void QueuePacket(ISerializablePacket packet, params int[] excludedIds) =>
         this.priorityQueue.Enqueue(new() { Packet = packet, ExcludedIds = excludedIds }, 1);

    public void QueuePacketToWorld(IWorld world, ISerializablePacket packet, params int[] excludedIds)
    {
        this.priorityQueue.Enqueue(new() { Packet = packet, ToWorld = world, ExcludedIds = excludedIds }, 1);
    }

    public void QueuePacketToWorld(IWorld world, int priority, ISerializablePacket packet, params int[] excludedIds) =>
        this.priorityQueue.Enqueue(new() { Packet = packet, ExcludedIds = excludedIds, ToWorld = world }, priority);

    public void QueuePacket(ISerializablePacket packet, int priority, params int[] excludedIds) =>
        this.priorityQueue.Enqueue(new() { Packet = packet, ExcludedIds = excludedIds }, priority);

    public void Broadcast(ISerializablePacket packet, params int[] excludedIds)
    {
        foreach (var player in this.server.Players.Cast<Player>().Where(player => !excludedIds.Contains(player.EntityId)))
            player.client.SendPacket(packet);
    }

    public void BroadcastToWorldInRange(IWorld toWorld, VectorF location, ISerializablePacket packet, params int[] excludedIds)
    {
    ...
    public void QueuePacketToWorldInRange(IWorld toWorld, VectorF location, ISerializablePacket packet, params int[] excludedIds)
    {
        if (toWorld is not World world)
            return;
    ...
    public void BroadcastToWorld(IWorld toWorld, ISerializablePacket packet, params int[] excludedIds)
    {
        if (toWorld is not World world)
            return;
    ...
    {
        public required ISerializablePacket Packet { get; init; }
...
public interface IPacketBroadcaster
{
    ...
    public void BroadcastToWorldInRange(IWorld world, VectorF location, ISerializablePacket packet, params int[] excludedIds);
    ...
    public void QueuePacketToWorld(IWorld toWorld, ISerializablePacket packet, params int[] excludedIds);

    public void QueuePacketToWorldInRange(IWorld world, VectorF location, ISerializablePacket packet, params int[] excludedIds);
    ...
    /// <param name="packet">The packet to send.</param>
    /// <param name="excludedIds">The list of entity ids to exlude from the broadcast.</param>
    public void QueuePacketToWorld(IWorld toWorld, int priority, ISerializablePacket packet, params int[] excludedIds);
