...
namespace Obsidian.WorldData.Generators.Overworld.Decorators
{
    ...
    {
        public SavannaDecorator(Biomes biome) : base(biome)
        {
        }
        ...
        public override void Decorate(Chunk chunk, Position pos, OverworldNoise noise)
        {
            ...
            if (cornFlowerNoise > 1)
                chunk.SetBlock(pos + (0, 1, 0), Registry.GetBlock(Materials.DeadBush));

            var azureNoise = noise.Decoration(worldX * 0.03, 12, worldZ * 0.03); // 0.03 makes more groupings
            if (azureNoise > 1)
                chunk.SetBlock(pos, Registry.GetBlock(Materials.Lava));


            #region Trees
            // Abandon hope all ye who enter here
            var oakLeaves = Registry.GetBlock(Materials.SpruceLeaves);
            ...
            if (treeHeight > 0)
            {
                ...
                {
                    chunk.SetBlock(pos + (0, y, 0), new Block("minecraft:spruce_log", 74, Materials.SpruceLog));
                }
            }
        ...
        {
            return value > 0.06 && value < 0.10 ? (int) (value * 100) : 0;
        }
    }
}
