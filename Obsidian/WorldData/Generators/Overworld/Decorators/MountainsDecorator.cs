...
public class MountainsDecorator : BaseDecorator
{
    ...
    public override void Decorate()
    {
        ...
        if (pos.Y > 120)
        {
            chunk.SetBlock(pos, BlocksRegistry.Get(Material.SnowBlock));
            return;
        }
        ...
        if (grassNoise > 0 && grassNoise < 0.5) // 50% chance for grass
            chunk.SetBlock(pos, BlocksRegistry.Get(Material.Cobblestone));
        ...
        if (azureNoise > 1)
            chunk.SetBlock(pos, BlocksRegistry.Get(Material.Snow));
    }
}
