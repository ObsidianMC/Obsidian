...
namespace Obsidian.WorldData.Generators.Modules
{
    ...
    {
        private static readonly float[] vectorTable =
        {
        ...
        public static float GradientCoherentNoise3D(float x, float y, float z, int seed)
        {
            float f0 = MathF.Floor(x);
            float f1 = MathF.Floor(y);
            float f2 = MathF.Floor(z);
            ...
            float sx = MathHelper.SCurve5(x - f0);
            float sy = MathHelper.SCurve5(y - f1);
            float sz = MathHelper.SCurve5(z - f2);
            ...
            ix1 = MathHelper.Lerp(n1, n0, sx);
            iy0 = MathHelper.Lerp(ix0, ix1, sy);
            ...
            ix1 = MathHelper.Lerp(n1, n0, sx);
            iy1 = MathHelper.Lerp(ix0, ix1, sy);

            return MathHelper.Lerp(iy0, iy1, sz);
        }

        public static float GradientNoise3D(float x, float y, float z, int nx, int ny, int nz, int seed)
        {
            int vectorIndex;
            unchecked
            {
                vectorIndex = nx * 1619 + ny * 31337 + nz * 6971;
                vectorIndex += (seed * 1013);
                vectorIndex ^= (vectorIndex >> 8);
                vectorIndex &= 0xff << 2;
            }

            Vector3 gradient = Unsafe.As<float, Vector3>(ref Unsafe.Add(ref MemoryMarshal.GetArrayDataReference(vectorTable), vectorIndex));
            Vector3 distance = new(x - nx, y - ny, z - nz);
            return Vector3.Dot(gradient, distance) * 2.12f;
        }
    }
}
