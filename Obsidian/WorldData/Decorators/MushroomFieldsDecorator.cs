...
{
    public MushroomFieldsDecorator(Biome biome, IChunk chunk, Vector surfacePos, GenHelper helper) : base(biome, chunk, surfacePos, helper)
    {
    ...
    public override void Decorate()
    {
        ...
        if (poppyNoise > 1)
            Chunk.SetBlock(Position + (0, 1, 0), BlocksRegistry.Poppy);
        ...
        if (cornFlowerNoise > 1)
            Chunk.SetBlock(Position + (0, 1, 0), BlocksRegistry.Cornflower);

        var azureNoise = Noise.Decoration.GetValue(worldX * 0.03, 12, worldZ * 0.03); // 0.03 makes more groupings
        if (azureNoise > 1)
            Chunk.SetBlock(Position + (0, 1, 0), BlocksRegistry.AzureBluet);
    }
}
