...
public class World : IWorld
{
    ...
    public Region GetRegionForChunk(int chunkX, int chunkZ)
    {
        ...
        return Regions.SingleOrDefault(x => x.Key == value).Value;
    }
    ...
    public async Task DoWorldTickAsync()
    {
        ...
        if (oldLevel != rainLevel)
        {
            ...
            Server.BroadcastPacket(new ChangeGameState(ChangeGameStateReason.RainLevelChange, rainLevel));
            if (rainLevel < 0.3f && rainLevel > 0.1f)
                Server.BroadcastPacket(new ChangeGameState(LevelData.Raining ? ChangeGameStateReason.BeginRaining : ChangeGameStateReason.EndRaining));
        }

        if (LevelData.Time % (20 * Server.Config.TimeTickSpeedMultiplier) == 0)
        {
            // Update client time every second / 20 ticks
            Server.BroadcastPacket(new TimeUpdate(LevelData.Time, LevelData.Time % 24000));
        }
    ...
    public async Task<bool> LoadAsync(DimensionCodec codec)
    {
        ...
        var dataPath = Path.Join(Server.ServerFolderPath, ParentWorldName ?? Name, "level.dat");
    ...
    public async Task SaveAsync()
    {
        ...
        if (worldFile.Exists)
        {
            worldFile.CopyTo($"{LevelDataFilePath}.old");
            worldFile.Delete();
        }
    ...
    public async Task<Region?> LoadRegionAsync(int regionX, int regionZ)
    {
        ...
        if (!Regions.TryAdd(value, null))
        {
            if (Regions[value] is not null)
            {
                return Regions[value];
            }
        }

        var region = new Region(regionX, regionZ, FolderPath);
        if (await region.InitAsync())
        {
            _ = Task.Run(() => region.BeginTickAsync(Server.cts.Token));
    ...
    public Task SpawnExperienceOrbs(VectorF position, short count = 1) =>
        Server.QueueBroadcastPacketAsync(new SpawnExperienceOrb(count, position));

    public Task SpawnPaintingAsync(Vector position, Painting painting, PaintingDirection direction, Guid uuid = default) =>
        Server.QueueBroadcastPacketAsync(new SpawnPainting(uuid == Guid.Empty ? Guid.NewGuid() : uuid, painting.Id, position, direction));
    ...
    internal void Init(DimensionCodec codec, string? parentWorldName = null)
    {
        ...
        {
            FolderPath = Path.Combine(Server.ServerFolderPath, Name);
            LevelDataFilePath = Path.Combine(FolderPath, "level.dat");
            PlayerDataPath = Path.Combine(FolderPath, "playerdata");
        }
        else
        {
            FolderPath = Path.Combine(Server.ServerFolderPath, parentWorldName, Name);
            LevelDataFilePath = Path.Combine(Server.ServerFolderPath, parentWorldName, "level.dat");
            PlayerDataPath = Path.Combine(Server.ServerFolderPath, parentWorldName, "playerdata");
        }
